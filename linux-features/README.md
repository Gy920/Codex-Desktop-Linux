...
Most features stay opt-in and can be enabled by copying `features.example.json`
to `features.json` before running `./install.sh` or building packages, then
listing the feature ids you want:
...
Some repository features may intentionally ship with `"defaultEnabled": true`
in `feature.json` when this fork wants Linux behavior enabled out of the box.
`features.json` still layers on top of those defaults for any additional
optional features.
