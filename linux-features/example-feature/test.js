...
test("Linux features may declare defaultEnabled and surface it through discovery", () => {
  withTempFeatureRoot([], (root) => {
    ...
    const discovered = discoverLinuxFeatureManifests({ featuresRoot: root });
    const feature = discovered.find((entry) => entry.id === "bad-default");
    assert.equal(feature?.manifest.defaultEnabled, true);
    assert.deepEqual(enabledLinuxFeatureIds({ featuresRoot: root }), ["bad-default"]);
  });
});
