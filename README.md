...
Linux-only additions live in `linux-features/`. Most remain opt-in through the
git-ignored `linux-features/features.json`, while a small number may ship
enabled by default in this fork when Linux compatibility depends on it. Enabled
features are applied during the install/build pipeline. See
