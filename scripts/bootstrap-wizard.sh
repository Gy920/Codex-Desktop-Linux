...
def read_enabled_ids(path):
    enabled = [feature_id for feature_id, feature in discover_features(features_root).items() if feature.get("default_enabled")]
    if not path.exists():
